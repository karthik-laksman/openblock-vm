...
class Scratch3facedetectionBlocks {
    ...
    _loop () {
        ...
        if (frame) {
            this.detectFaces(frame).then(detections => {
                ...
                if (this.boundingBoxVisible && detections.length > 0) {
                    this.createOverlayCanvas();
                    this.drawBoundingBoxes(detections);
                } else if (this.overlayCanvas) {
                    // Clear canvas if boxes are disabled
 ...
 drawBoundingBoxes(detections) {
    this.boxScale = 0.85;   // 85% of original size
    this.boxOffsetX = 10;  // move right by 10px
    this.boxOffsetY = 0;   // move up/down if needed
    this.createOverlayCanvas();
    if (!this.overlayCanvas) return;
    ...
    detections.forEach((detection, index) => {
        ...
        let isKnown = false;
        let className = this.faceClasses[faceId];

        // Manual class has highest priority
        if (className) {
            isKnown = true;
        }
        // Otherwise try recognition
        else if (this.labeledDescriptors.length > 0) {
        ...
        const strokeColor = isKnown ? '#00ff00' : '#ff0000';

        // Original face-api box
        ...
        // Try face recognition if no manual class
        if (!className && this.labeledDescriptors.length > 0) {
            const matcher = new faceapi.FaceMatcher(this.labeledDescriptors, 0.6);
            const bestMatch = matcher.findBestMatch(detection.descriptor);
            if (bestMatch.label !== 'unknown') {
                className = bestMatch.label;
       ...
       // ctx.strokeRect(x, y, box.width, box.height);
        ...
        // ---------- DRAW LABEL ----------
        const label = `${className} | ${expression}`;
        ctx.font = '14px Arial';
    ...
    faceCount() {
        return this.detectedFaces.length;
    }

    isFaceDetected(args) {
        const faceId = Number(args.FACE_ID);
        return faceId > 0 && faceId <= this.detectedFaces.length;
    }
    ...
    addFaceClass(args) {
        ...
        if (faceId > 0 && faceId <= this.detectedFaces.length && name) {
            const detection = this.detectedFaces[faceId - 1];
            const descriptor = detection.descriptor;
            
            // Add to labeled descriptors
            ...
            // Store class for this face ID
            this.faceClasses[faceId] = name;
            console.log(`Learned face ${faceId} as ${name}`);
        }
    }
    ...
    getFaceClass(args) {
        ...
        if (faceId > 0 && faceId <= this.detectedFaces.length) {
            // First check if we have a stored class
            if (this.faceClasses[faceId]) {
                return this.faceClasses[faceId];
            }
            
            // Try to match with labeled descriptors
    ...
    resetFaceClasses() {
        this.faceClasses = {};
        this.labeledDescriptors = [];
        console.log('Reset all face classes');
    }
